    ...
    <article className="card overflow-hidden">
      ...
      <div className="grid gap-3 p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          <button className="rounded-full p-2 text-ocean hover:bg-mist" aria-label="Ajouter aux favoris"><Heart className="h-5 w-5" /></button>
        </div>
        ...
        <button className="btn-primary w-full" disabled={!variant} onClick={() => variant && add(variant.id).then(() => toast.success('Produit ajoute au panier'))}>
