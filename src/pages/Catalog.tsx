  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-8 px-4 py-8 lg:grid-cols-2">
      ...
      <div className="grid content-start gap-5">
        ...
        <div className="flex items-center gap-3"><input className="input max-w-24" type="number" min={1} max={variant?.inventory?.available_quantity || 1} value={qty} onChange={(e) => setQty(Math.max(1, Number(e.target.value)))} /><button className="btn-primary" disabled={!variant || (variant.inventory?.available_quantity || 0) < 1} onClick={() => variant && add(variant.id, qty).then(() => toast.success('Panier mis a jour'))}><ShoppingCart className="h-4 w-4" />{(variant?.inventory?.available_quantity || 0) > 0 ? 'Ajouter au panier' : 'Rupture de stock'}</button><a className="btn-secondary" href={`https://wa.me/?text=${encodeURIComponent(product.name)}`}><Share2 className="h-4 w-4" />Partager</a></div>
